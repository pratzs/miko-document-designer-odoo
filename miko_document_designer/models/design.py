...
# The line table, in every spelling core actually uses. `o_main_table` is the
# convention and covers the invoice, the quotation, the sales order and the
# purchase order - but stock's delivery slip and picking operations do not carry
# it and identify their tables by a `name` attribute instead, which does survive
# into the rendered HTML. Anything a merchant writes themselves picks the
# convention up for free by using the class.
LINE_TABLE_SELECTORS = (
    '.o_main_table',
...
# Reports core forgets to stamp with a company. `stock.report_picking` renders
# its article as `article o_report_layout_standard` with no `o_company_N_layout`
# at all, so a stylesheet scoped by company reaches every document except that
# one. This selector catches exactly those, and only those.
UNSCOPED_ARTICLE = '.article:not([class*="o_company_"])'


def build_css(design, scope=None, selector=None):
    ...
    `selector` overrides it with a raw selector, for the reports core does not
    stamp with a company at all.
    """
    design = merged(design)
    if not design.get('active'):
        return ''
    sel = selector or ('.%s' % scope)
    # One rule per table spelling, joined into a single selector list, so a
    # delivery slip is designed exactly like an invoice without a second pass.
    tables = ['%s %s' % (sel, each) for each in LINE_TABLE_SELECTORS]
    out = []

    def rule(suffix, body):
        """Emit one rule covering every line-table spelling."""
        return '%s { %s }' % (
            ', '.join('%s%s' % (table, suffix) for table in tables), body)

    table = tables[0]  # only for rules that are the same for every table
    ...
    if design['table_font_size']:
        out.append(rule('', 'font-size: %s;' % _px(design['table_font_size'])))

    padding = DENSITY_PADDING.get(design['density'])
    if padding:
        out.append(rule(' > thead > tr > th', 'padding: %s;' % padding))
        out.append(rule(' > tbody > tr > td', 'padding: %s;' % padding))

    grid = GRID_RULES.get(design['grid'])
    if grid:
        out.append(rule(' > thead > tr > th', grid))
        out.append(rule(' > tbody > tr > td', grid))
    ...
    if design['zebra']:
        ...
        out.append(rule(
            ...
            '-webkit-print-color-adjust: exact;' % design['zebra_color']))

    if design['row_numbers']:
        skip = ''.join(':not(.%s)' % klass for klass in NON_PRODUCT_ROW_CLASSES)
        out.append(rule(' > tbody', 'counter-reset: miko-line;'))
        out.append(rule(
            ' > tbody > tr%s > td:first-child:before' % skip,
    ...
    if design['head_bg'] or design['head_color']:
        ...
        out.append(rule(' > thead > tr > th', ' '.join(decl)))
