...
# The same set of line tables the stylesheet targets, as one xpath. Stock's
# tables are found by their `name` attribute, which does survive into the
# rendered HTML - checked against a real delivery slip rather than assumed.
MAIN_TABLE = (
    ".//table[contains(concat(' ', normalize-space(@class), ' '),"
    " ' o_main_table ')"
    " or @name='stock_move_table'"
    " or @name='stock_move_line_table']"
)
...
def inject_after_totals(article, fragment_html):
    ...
    `div#total` is core's own id and is in the invoice, the quotation, the sales
    order and the purchase order on every series. **A document without one gets
    nothing at all**: a delivery note has lines and a layout but no totals, and
    an earlier version that fell back to appending at the end of the article
    printed "How to pay us" with a bank account on a packing slip. Found by
    rendering a real delivery slip and looking at it.

    Returns True when the block was placed.
    """
    ...
    if not totals:
        return False
    totals[-1].addnext(lxml_html.fragment_fromstring(fragment_html))
    return True
