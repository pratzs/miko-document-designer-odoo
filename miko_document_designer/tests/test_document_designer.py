...
class TestDesignCss(common.TransactionCase):
    ...
    def test_table_font_size_targets_the_line_table(self):
        ...
        rule = [line for line in css.splitlines() if 'font-size: 9px' in line][0]
        self.assertIn('.%s .o_main_table' % SCOPE, rule)
        self.assertIn('table[name="stock_move_table"]', rule,
                      "one selector list covers every line table core writes")
    ...
    def test_stock_line_tables_are_designed_too(self):
        """Core does not put `o_main_table` on the delivery slip.

        It names those tables instead, and the name survives into the rendered
        HTML. A design that only knew the class left packing slips undesigned,
        which the first real delivery-slip render showed immediately.
        """
        css = build_css(dict(DEFAULTS, zebra=True, row_numbers=True), SCOPE)
        self.assertIn('table[name="stock_move_table"]', css)
        self.assertIn('table[name="stock_move_line_table"]', css)
        for selector in ('.o_main_table', 'table[name="stock_move_table"]'):
            self.assertIn('%s > tbody > tr:nth-child(even) > td'
                          % ('.%s %s' % (SCOPE, selector)), css)

    def test_a_report_with_no_company_class_is_still_designed(self):
        """`stock.report_picking` renders without `o_company_N_layout`.

        Every other report carries it, so a stylesheet scoped by company reached
        every document except that one - which a real render of a receipt showed
        as the single undesigned document in the set.
        """
        from odoo.addons.miko_document_designer.models.design import (
            UNSCOPED_ARTICLE,
        )
        css = build_css(dict(DEFAULTS, zebra=True), selector=UNSCOPED_ARTICLE)
        self.assertIn('.article:not([class*="o_company_"])', css)
        self.assertIn('nth-child(even)', css)
...
class TestDocumentHtml(common.TransactionCase):
    ...
    def test_a_block_with_no_totals_is_not_placed_at_all(self):
        """Superseded the old fallback, which appended at the end of the article.

        That fallback is what printed a bank block on a delivery note.
        """
        article = document_html.parse('<div class="article"><p>Body</p></div>')
        placed = document_html.inject_after_totals(
            article, '<div class="marker">X</div>')
        self.assertFalse(placed)
        self.assertNotIn('marker', document_html.serialise(article).decode())
...
<t t-name="miko_document_designer.test_document">
    ...
    def test_a_document_with_no_totals_block_gets_no_bank_block(self):
        """Payment details do not belong on a packing slip.

        A delivery note has lines and a layout but shows no totals, so it has no
        `div#total`. An earlier version fell back to appending at the end of the
        article and printed "How to pay us" with a bank account on a real
        packing slip - found by rendering one and looking at it.
        """
        article = document_html.parse(
            '<div class="article"><table class="o_main_table"><tbody>'
            '<tr><td>A line</td></tr></tbody></table></div>')
        placed = document_html.inject_after_totals(
            article, '<div class="miko-totals-extra">How to pay us</div>')
        self.assertFalse(placed)
        self.assertNotIn('How to pay us',
                         document_html.serialise(article).decode())
